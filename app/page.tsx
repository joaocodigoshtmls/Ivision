...
export default function HomePage() {
  return (<main className="space-y-10">
    ...
    <section className="grid gap-6 md:grid-cols-3">
      ...
      <Feature icon={<Play />} title="Ao vivo" desc="WebRTC quando disponível; fallback HLS." />
      <Feature icon={<ShieldCheck />} title="Segurança" desc="RBAC e auditoria." />
    </section>
  </main>);
}
function Feature({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (<div className="rounded-2xl border border-white/10 p-5">
    <div className="mb-3 flex items-center gap-2 text-zinc-300"><span className="rounded-xl bg-white/5 p-2">{icon}</span>
    <h3 className="font-medium text-white">{title}</h3></div><p className="text-sm text-zinc-400">{desc}</p></div>);
}
